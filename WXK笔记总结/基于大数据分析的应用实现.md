...
#运行第一个镜像
docker run -ti wxk554/spark:PM2.5mysql_secure_installation
...
docker commit -m "wget vim net-tools iputils-ping install" 容器ID wxk554/spark:PM2.5
...
$ exit 
    ...
    保存一个副本
$docker commit -m "zookeeper hadoop pyspark scala python java install" 容器ID wxk554/spark:PM2.5
...
终端 1:
    ...
    $ docker run -ti -h master wxk554/spark:PM2.5
    $ ifconfig #172.17.0.2

终端 2:

    $ docker run -ti -h slave1 wxk554/spark:PM2.5soft
    
    $ docker run -ti -h slave2 wxk554/spark:PM2.5
    $ ifconfig #172.17.0.4
...
##  编写集群节点启动脚本
启动 wxk554/spark:PM2.5

$ docker run -ti wxk554/spark:PM2.5
...
$ docker commit -m "zookeeper hadoop spark scala install" 容器ID wxk554/spark:PM2.5
...
启动master：
    ...
    $  docker run --privileged -itd --name=master -h master wxk554/spark:PM2.5 /usr/sbin/init 
    $ ./home/wxk/script/run_master.sh

启动slave1：

    启动 Slave1 节点
    $ docker run --privileged -itd --name=slave1 -h slave1 wxk554/spark:PM2.5 /usr/sbin/init
...
启动slave2：
    ...
    启动 Slave2 节点
    $ docker run --privileged -itd --name=slave2 -h slave2 wxk554/spark:PM2.5 /usr/sbin/init
...
docker run --privileged -itd --name master --add-host host:172.17.0.1 --add-host slave1:172.17.0.3 --add-host slave2:172.17.0.4 -h master wxk554/spark:PM2.5 /usr/sbin/init

docker exec -it -u wxk master /bin/bash



docker run --privileged -itd --name slave1 --add-host host:172.17.0.1 --add-host master:172.17.0.2  --add-host slave2:172.17.0.4 -h slave1 wxk554/spark:PM2.5 /usr/sbin/init

docker exec -it -u wxk slave1 /bin/bash



docker run --privileged -itd --name slave2 --add-host host:172.17.0.1 --add-host master:172.17.0.2 --add-host slave1:172.17.0.3  -h slave2 wxk554/spark:PM2.5 /usr/sbin/init
...
保存一个副本
$docker commit -m "zookeeper hadoop pyspark scala python java spark-historyserver install" 容器ID wxk554/spark:PM2.5
...
docker run --privileged -itd --name=master -h master wxk554/spark:PM2.5 /usr/sbin/init七、遇到的问题以及解决方法
...
docker run --privileged -itd --name=slave2 -h slave2 wxk554/spark:PM2.5 /usr/sbin/init
docker run --privileged -itd --name=test -h test wxk554/spark:PM2.5 /usr/sbin/init
