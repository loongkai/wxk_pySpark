...
```
sc.textFile("hdfs://master:8020/hello.txt").collect()
```
...
> 也可以通过web界面查看： http://master:8088/cluster 
...
```
curl -k -X POST --data "action=login&username=azkaban&password=azkaban" http://master:9081
```
...
```
curl -k -X POST --data "session.id=97410ddc-d9c8-414d-bd27-389bc692cf7a&name=projectajax&description=projectajax" http://master:9081/manager?action=create
```
...
```
curl -k --get --data "session.id=97410ddc-d9c8-414d-bd27-389bc692cf7a&delete=true&project=nanshou" http://master:9081/manager
```

### 4.上传 Project Zip

```
curl -k -i -H "Content-Type: multipart/mixed" -X POST --form 'session.id=6dd538ab-befe-45f1a767-6933287e68d8' --form 'ajax=upload' --form 'file=@hadoop.zip;type=application/zip' --form 'project=ipget' http://master:9081/manager
```

### 5. 获取工作流 

```
curl -k --get --data "session.id=1590ab7a-503a-4430-bf39-21391b83d616&ajax=fetchprojectflows&project=ipget" http://master:9081 /manager
```

### 6.提取流程作业

```
curl -k --get --data "session.id=1590ab7a-503a-4430-bf39-21391b83d616&ajax=fetchflowgraph&project=ipget&flow=hadoop" http://master:9081/manager
```
